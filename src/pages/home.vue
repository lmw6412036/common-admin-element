    ...
    <el-container>
      <el-aside width="200px" class="aside" :style="asideStyle">
        ...
        <el-menu
          ...
          <el-submenu :index="menu.id+''" :key="menu.id" v-for="menu in menus">
            <template slot="title">
              <i class="el-icon-location"></i>
              <span>{{menu.name}}</span>
            </template>
            <el-menu-item v-for="sub in menu.subMenus" :index="sub.url" :key="sub.id">{{sub.name}}</el-menu-item>
          </el-submenu>
        </el-menu>
      </el-aside>
      <el-container>
  ...
  export default {
    ...
    methods: {
      async getAdminMenu() {
        ...
        if (ret.errno == 0) {
          this.changeMenus(ret.data);
        }
      },
      ...mapMutations({
        changeMenus: types.MENUS
      })
    }
  };
</script>
